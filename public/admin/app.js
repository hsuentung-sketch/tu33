    ...
    items.forEach((it, idx) => {
      const row = el('div', { class: 'field row', style: 'gap:6px;align-items:flex-end;' },
        ...
        el('div', { class: 'field', style: 'max-width:90px;' },
          el('label', {}, '折讓數量'),
          el('input', { type: 'number', min: '0', step: '1', value: String(it.quantity),
  ...
  const modal = el('div', { class: 'modal', style: 'max-width:720px;' },
    ...
    el('div', { class: 'body' },
      ...
      el('div', { style: 'font-size:12px;color:var(--muted);margin-bottom:4px;' },
        '輸入各品項折讓數量（0 表示不折讓）。送出會產生 G0401 XML 寫入 Turnkey 匯入目錄。'),
      itemsHost,
    ),
    errBox,
    el('div', { class: 'actions' },
...
async function openEinvoiceIssueModal(ar, onSaved) {
  ...
  const zeroReasonInput = el('select', {
    ...
    el('option', { value: '' }, '（不適用）'),
    el('option', { value: '71' }, '71 外銷貨物'),
    el('option', { value: '72' }, '72 與外銷有關之勞務'),
    el('option', { value: '73' }, '73 依法免徵營業稅之外銷貨物'),
    el('option', { value: '74' }, '74 銷售與保稅區之貨物勞務'),
    el('option', { value: '75' }, '75 國際運輸'),
    el('option', { value: '76' }, '76 國際運輸用之船舶航空器'),
    el('option', { value: '77' }, '77 保稅區營業人銷售'),
    el('option', { value: '78' }, '78 保稅區營業人銷售給國內'),
    el('option', { value: '79' }, '79 其他'),
  );
