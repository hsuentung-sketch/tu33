...
## 踩過的坑備忘
- **DB 遷移必須審計全部平台服務**：2026-06-08 發現 Supabase→Neon DB 遷移後，原 Supabase 專案因無 DB 活動被自動刪除，Storage bucket 與已上傳產品文件一併消失。教訓：遷移 DB 前必須列出平台所有服務（DB / Storage / Auth / Edge Functions），逐一確認是否仍有依賴。v2.16.0 改為 `fileData Bytes` 存 DB，零外部儲存依賴。
- Express 5：mount 順序重要，`/pdf` 和 `/doc` **必須在 authMiddleware 之前**
