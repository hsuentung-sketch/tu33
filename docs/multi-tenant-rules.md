...
## 9. Tenant 設定 source of truth（`tenants/` 目錄）

每個 Tenant 的差異設定存放在 `tenants/<客戶>/` 目錄（不寫死在 src/）：

| 檔案 | 用途 |
|------|------|
| `modules.json` | 此 Tenant 啟用的模組清單 |
| `settings.json` | 行為差異（LINE channel、billing days 等） |
| `overrides.md` | 偏離行業模板的差異說明 |
| `CHANGELOG.md` | 設定變更歷史 |
...
注意事項：
- `tenants/` 是**文件化的設定參考層**，目前不影響 runtime（runtime 由 `Tenant.settings` DB 欄位負責）
- 未來若需程式動態讀取，由 DB 負責；`tenants/*.json` 是 source of truth 文件
- **敏感值**（LINE Channel Secret / Access Token 等）不進這些 json，走 Fly secrets + Bitwarden
